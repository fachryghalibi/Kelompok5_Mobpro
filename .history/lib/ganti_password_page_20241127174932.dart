...
import 'package:tubesmopro/service/ganti_passwrod_service.dart';
...
class _ChangePasswordPageState extends State<ChangePasswordPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _handlePasswordChange() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        final gantiPassword = GantiPassword();
        final response = await gantiPassword.changePassword(
          widget.userId,
        ...
        if (response['status'] == 'success') {
          ...
          if (mounted) {
            _showErrorNotification(response['message'] ?? 'Terjadi kesalahan saat mengubah kata sandi');
          }
        }
      } catch (e) {
        if (mounted) {
          String errorMessage = 'Terjadi kesalahan saat mengubah kata sandi';
          if (e is Exception) {
